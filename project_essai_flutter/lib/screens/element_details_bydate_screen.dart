...
class ElementDetailsByDateScreen extends StatefulWidget {
  ...
  ElementDetailsByDateScreen({required this.element, required this.token, this.inspection,required this.inspectionDate});

  @override
  _ElementDetailsByDateScreenState createState() => _ElementDetailsByDateScreenState();
}

class _ElementDetailsByDateScreenState extends State<ElementDetailsByDateScreen> {
      String? _codeDemande;   
 ...
 TextEditingController idIntervenantController = TextEditingController();
  ...
  void initState() {
    ...
    inspectionObservationController.text = widget.element.observation ?? ''; 
      ...
      idIntervenantController.text = widget.inspection?.numeroInter ?? '';
...
Future<void> _showInspectionForm({bool isUpdate = false}) async {
  ...
  if (isUpdate) {
        isOkSelected =widget.inspection!.resultats as bool;

    inspectionObservationController.text = widget.inspection?.observation ?? '';
    typeMaintenanceController.text = widget.inspection?.typeMaintenance ?? '';
    numeroInterController.text = widget.inspection?.numeroInter ?? '';
numeroInterController.text = (widget.inspection?.idIntervenant?.toString()) ?? '';
  }
            ...
            onPressed: () async {
              if (_formKey.currentState?.validate() ?? false) {
                final inspection = Inspection(
                  ...
                  dateInspection: widget.inspectionDate,
                   observation: inspectionObservationController.text.trim(), 
                  codeElementSection: widget.element.codeElement ?? '',
                  idIntervenant: numeroInterController.text.trim().isNotEmpty 
                      ? int.tryParse(numeroInterController.text.trim()) 
                      : null,
                  codeEquipement: widget.element.codeSection ?? '',
                  dateElement: widget.inspectionDate,
                  ...
                  createDateTime: isUpdate ? (widget.inspection?.createDateTime ):widget.inspectionDate,
                  lastUpdateUser:Provider.of<User>(context, listen: false).matricule,
                  lastUpdateDateTime: widget.inspectionDate,
...
void updateObservationButton() async {
  final apiService = Provider.of<ApiService>(context, listen: false);
     print('Date d\'inspection passée: ${widget.inspectionDate}');
...
void _handleInterventionButton() async {
  ...
  DateTime today = widget.inspectionDate;
              ...
              children: <Widget>[
                ...
                DropdownButtonFormField<int>(
                  ...
                  items: _idPanneOptions.map((int value) {
                    return DropdownMenuItem<int>(
                      value: value,
                      child: Text('${_idPanneDescriptions[value]} ($value)'),
                    );
                  }).toList(),
                  onChanged: (newValue) {
            ...
            onPressed: () async {
              if (_formKey.currentState?.validate() ?? false) {
                final demandeintervention = DemandeInterventions(
                  ...
                  dateHeureDemande: widget.inspectionDate,
                ...
                codeEquipement:codeEquipementController.text.trim(),
                  ...
                  createDateTime: isUpdate ? existingDemande?.createDateTime: widget.inspectionDate,
                  lastUpdateUser: Provider.of<User>(context, listen: false).matricule,
                   lastUpdateDateTime: widget.inspectionDate,
                  createMachine:widget.element.createMachine,
                  dateCreateReel: widget.inspectionDate,
